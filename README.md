...
> Desktop app to monitor GitHub Pull Requests in real-time — built with Tauri 2 + React 19.
...
Keep an eye on all your open PRs across multiple repositories without ever leaving your desktop. PR Dashboard polls GitHub (or GitHub Enterprise) on a configurable interval and gives you:
...
| **Persistent config** | Token, repos, interval stored in `%APPDATA%` via `tauri-plugin-store` |
...
| **GitHub URL** | Your GitHub instance (default: `https://github.sinequa.com`) |
| **Refresh interval** | Polling interval in seconds (default: 60) |
| **Repositories** | List of `owner/repo` pairs to monitor |
...
The app uses a single **GraphQL** call per repository to fetch the 50 most recently updated open PRs (including review decisions and approvers). A lightweight **REST** call is made only when a PR disappears from the open list, to distinguish a merge from a simple close.
